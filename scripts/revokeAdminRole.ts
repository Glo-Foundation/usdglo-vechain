...
async function main() {
  ...
  const has_role = await token.hasRole(admin_role, walletToRevokeRole);

  console.log("has admin role: ", has_role);

  const revokeAdminRole = await token.revokeRole(admin_role, walletToRevokeRole);


  const now_has_role = await token.hasRole(admin_role, walletToRevokeRole);
