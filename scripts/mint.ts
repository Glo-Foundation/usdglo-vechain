...
async function main() {
  ...
  const amountToMint = 10n * (10n ** 18n);

  const recipientBalance = await token.balanceOf(recipient);

  console.log("recipient current balance: ", recipientBalance);

  const mintToAddress = await token.mint(recipient, amountToMint);

  const recipientNewBalance = await token.balanceOf(recipient);

  console.log("recipient new balance: ", recipientNewBalance);
